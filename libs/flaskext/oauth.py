...
class OAuthClient(oauth2.Client):
    ...
    def request_new_token(self, uri, callback=None):
        ...
        if gethostname().lower() == 'mclovin':
            import socks
            self.proxy_info = httplib2.ProxyInfo(socks.PROXY_TYPE_HTTP, 'www-proxy.cs.tcd.ie', 8080)
...
class OAuthRemoteApp(object):
    ...
    def generate_request_token(self, callback=None):
        ...
        if resp['status'] != '200' and resp['status'] != 200:
            raise OAuthException('Failed to generate request token - %s %s' % (resp['status'], str(content)))
